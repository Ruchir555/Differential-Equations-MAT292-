...
% 
% Opening the m-file lab1.m in the MATLAB editor, step through each part 
% using cell mode to see the results. Compare the output with the PDF, which was 
% generated from this m-file.
% 
% There are twelve (12) exercises in this lab that will be graded. Write 
...
% & Sinnamon MAT292 Fall 2016, Sinnamon & Sousa Based on original from MAT292 
% Fall 2012, Hart & Pym
%% Student Information
...
% 
% Only include the error message as a comment (with the percent signs), DO 
% NOT INCLUDE THE CODE IN YOUR SUBMISSION.
%% Plotting a Vector
...
% y = [-25, -9, -1, 0, 4, 16, 36]
% 
% And plot |x| against |y|. Your submission should show both the creation 
% of the vectors and the figure, as in the example.
%%
x = [-5, -3, -1, 0, 2, 4, 6] ;
y = [-25, -9, -1, 0, 4, 16, 36];
figure();
...
% of 0 and 1, the values would be: h = (b - a)/(N-1) and a = x(1) = a + 0 * h 
% < x(2) = a+ 1 * h < ... < x(j) = a +(j-1) * h < ...< x(N) = b
% 
...
% memory for future use'. Preallocation can be done with the command |x = zeros(N,1);|, 
...
N1 = 101;   
h1 = (7+3)/(N-1);
x1 = zeros(N1,1);    %Preallocate the array

for j = 1:N1
    x1(j) = ((j-1) * h)-3;
end

disp(x1);    %Verify x1 is displaying correctly

y1 = zeros(N1, 1);   %Preallocate the array

for i = 1:N1 
    y1(i) = abs(x1(i))*x1(i);
end

disp(y1);    %Verify y1 is displaying correctly
figure();
plot(x1,y1)
...
% see a section:
% 
% LEGEND(...,'Location',LOC) adds a legend in the specified location, LOC, 
% with respect to the axes. LOC may be either a 1x4 position vector or one of 
...
% vectorization, which proccesses the entire vector as a single object. For example, 
% given a vector |x|, we can get the sine of the values in |x| with the command 
% |sin(x)|, which outputs another vector. This works with all built in MATLAB 
% functions. 
% 
...
% 
% However, we can still demonstrate the speed advantage, and the benefit 
% to clarity is already evident.
% 
% Timing is done with the |tic|, |toc| commands, which enclose the region 
% we want to benchmark. The first output is the time for the construction done 
% by looping, the second output is the time for construction done with vectorization.
%%
...
% 
% Details: Create a vector |x| of 5000001 equally space points between -2 
% and 3, inclusive, using a for loop. Then time the construction of vector |y|, 
% whose values are arctan of x, by a for loop and by vectorization. Be sure to 
% clear  |y| between the two runs. (Hint: use the help file to find the command 
% for arctan!)
% 
...
N2 = 5000001;
h = (3+2)/(N2-1);
x = zeros(N2,1);    %Preallocate the array
y = zeros(N2,1);    % Preallocate the array
for i = 1:N2
    x(i) = ((i-1)*h);   
end

%disp(x);    % To verify the correct output of x. Note that this was disabled because of excess computation time.
...
xlabel('x');
ylabel('f(x)');
title('Example plot of two functions generated by vectorization');
...
% and y(1) = 1. Plot these three particular solutions on the same axes lettting 
% the independent variable range from 1 to 5. 
% 
...
% solution of the ODE and the three particular solutions. It should also have 
% the appropriately commented code.
%% *** Solution***
% The ODE can be solved by using an integrating factor. The general form of 
% the solution is:
% 
% $$y=\frac{t+C}{e^{t^2 } }$$
% 
% Where C is an arbitrary constant. Using the three initial conditions above, 
% the three unique values of C can be determined as
% 
%  C = -1-e, C = -1, and C = e-1. We can now plot these three solutions as 
% follows:
%%
clear t Y1 Y2 Y3;
t = linspace(1, 5, 100);    %Create the array

Y_1 = (t - 1 - exp(1)) ./ (exp(t.^2));
Y_2 = (t - 1) ./ (exp(t.^2));                          % Three solution curves
Y_3 = (t + exp(1) - 1) ./ (exp(t.^2));


figure();
plot(t, Y_1, t, Y_2, t, Y_3)
xlabel('t');
ylabel('y(t)');
title('Three solutions to the ODE: dy/dt + (2*t)*y = 1/exp(t^2)');
legend('(t-1-e) / (e^t^2)', '(t-1) / (e^t^2)', '(t+e-1) / (e^t^2)', 'Location','northeast');
%% 
% 
...
title('Exercise 6: Plot of an inline function')
xlabel('t');
ylabel('f(t)');
legend('(x^3 - abs(x)) / (Ln(x^2 +1)', 'location', 'southeast')
...
% This is done by creating an inline function of one variable from the existing 
% function, i.e.
% 
% g = @(y) f(x0, y)
% 
...
% good initial guess may require some experimentation. For this example, the guess 
...
guess = 100;
y = fzero(z, guess);
fprintf(' y(%g) = %g\n', x0, y);        %Same result in both cases
...
guess = 10;
z = fzero(w, guess);
fprintf('z(%g) = %g\n', y0, x)      %Same result in both cases
%% 
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4;
xvals = linspace(-2, 2);
yvals = zeros(size(xvals));     %Preallocate the array    
...
xlabel('x');
ylabel('y');
title('Exercise 9: Plotting a portion of an implicit equation');
%% Exercise 10
...
% Label your axes.
%% Solution:
% ****Note: Soluion was plotted both explicitly and implicitly due to the errors 
% given by fzero due to the behavior of the natural log function around zero (in 
% the implicit plot).*
% 
% **
%% Implicit solution:

% The general [implicit] form of the solution is : 
%                                          c = Ln|y| + sin(x) - x^3
% Substituting in the initial conditions, we get the constant c to be 0. So the particular solution in this case is:
%                                       0 = Ln|y| + sin(x) - x^3
...
for i = 1:length(xvals)
    yvals(i) = fzero(@(y) f(xvals(i), y), 1.5);     %Need to be careful with the guess
end
figure();
plot(xvals, yvals);
xlabel('x');
ylabel('y');
title('Exercise 10: Plotting a portion of an implicit equation from an ODE');
%% Explicit solution:
...
x = linspace(-1.5, 1.25);
y = zeros(size(xvals));     %Preallocate the array
...
xlabel('x');
ylabel('y');
title('Exercise 10: Plotting a portion of an explicit equation from an ODE');
%We can see that the graph has a zero at approximately x = -1.5.
%% Install iode
...
% for any solution.
%% Solution:
% The solution curves assume a U- shaped form above the x-axis. There us some 
% sort of vertical asymptotal
% 
% behaviour around x = 0, as well as some sort of horizontal asymptote behaviour 
% around y = 0. The portion of the graphs below the x-axis shows some sort of 
% parabolic behaviour.
% 
% as x approaches infinity, the curves for y>0 tend to infinity, while the 
% curves for y<0 tend to zero. The graph is symmetric about the y-axis. This may 
% be due to the nature of the general solution, i.e. it may have an x^2 term in 
% it. The general form of the solution is (implicitly): (1/y) = -1/(2x^2) + K,; 
% we can see that this contains an x^2 term, so that y(-x) = y(x). 
%% Exercise 12
...
% solution), determine the long-run behaviour of solutions as a function of |y_0|. 
% In other words, list the constant solutions that are approached as x goes to 
% infinity and how |y_0| determines which constant solution is approached.
% 
% If your numerically computed solutions ever appear to cross then change 
% the solution method from Euler to the, much better, Runge-Kutta method. Think 
% about why solutions are not permitted to cross. If they do then this indicates 
% a problem with the numerical solver.
%% Solution:
% Using the Euler's method, some solution curves appear to cross over. However, 
% this is not possible due to the nature of direction fields. The slope at any 
% one point is well-defined, thus crossing over cannot occur as this means more 
% than one slope at each point. Using the Runge-Kutta method we see that there 
% is horizontal asymptotal behaviour from the solution curves for a certaian range 
% of y_0 values. For y_0 less than -1, the curves tend to -1 as x tends to infinity, 
% and to negative infinity as x tends to negative infinity. For -1< y_0 < 1,
% 
% the solution curves tend to 1 as x tends to infinity, and to -1 as x tends 
% to negative infinity. For 1< y_0 < 2, the curves tend to 1 as x tends to infinity, 
% and to 2 as x tends to negative infinity. For y_0 greater than 2, the solution 
% curves tend to infinity as x tends to infinity, and to 2 as x tends to negative 
% infinity. 
% 
% 
...
% 
% Your submission should show the particular solution, in implicit form, 
% in the comments, the definition of the appropriate inline functions, arrays, 
